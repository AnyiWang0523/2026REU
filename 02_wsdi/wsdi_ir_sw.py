...
# Category: Irrigation - Surface Water
# Formula: (SW_Supply - Demand) / Demand
# Supply: IR_HUC12_SW_WD_m_2000_2020.csv (local CSV)
...
CSV_PATH   = os.path.join(BASE_DIR, "data", "CSV",    "IR_HUC12_SW_WD_monthly_2000_2020.csv")
OUTPUT_DIR = os.path.join(BASE_DIR, "output", "IR_SW")
os.makedirs(OUTPUT_DIR, exist_ok=True)

# ============================================================
# STEP 1: Load SW Data from Local CSV
# ============================================================
print("=" * 50)
print("STEP 1: Loading IR SW data from local CSV...")
print("=" * 50)

sw_raw = pd.read_csv(CSV_PATH, dtype=str)
print(f"✓ Load complete: {sw_raw.shape[0]} rows x {sw_raw.shape[1]} columns")
print(f"First 5 columns: {sw_raw.columns.tolist()[:5]}")
print(f"Sample rows:")
print(sw_raw.head(3).to_string())
...
sw_long["SW_Mgal"] = sw_long["SW_Mgal_day"] * sw_long["Month"].map(days_map)
sw_long["HUC4"]    = sw_long["HUC12"].str[:4]

sw_annual = (
    sw_long
    .groupby(["HUC4", "Year"])["SW_Mgal"]
    .sum()
    .reset_index()
    .rename(columns={"SW_Mgal": "SW_Supply"})
)

print(f"✓ Annual aggregation complete: {len(sw_annual)} records")
print(f"  Unique HUC4 units: {sw_annual['HUC4'].nunique()}")
print(sw_annual.head(5).to_string(index=False))
...
huc4_demand = huc4_ref.merge(
    ...
    right_on = ["HUC4", "Year"],
    how="left"
).rename(columns={"SW_Supply": "Demand"})[
...
wsdi["WSDI"] = (wsdi["SW_Supply"] - wsdi["Demand"]) / wsdi["Demand"]

print(f"\n✓ WSDI calculation complete: {len(wsdi)} records")
print("\nWSDI summary statistics:")
print(wsdi["WSDI"].describe().round(4))
print("\nSample results:")
print(wsdi[["HUC4","STATE","Year","SW_Supply","Demand","Demand_Year","WSDI"]].head(10).to_string(index=False))
...
out_csv = os.path.join(OUTPUT_DIR, "WSDI_IR_SW_HUC4_2000_2020.csv")
...
for yr in range(2000, 2021):
    ...
    merged = huc4_gdf.merge(
        sub[["HUC4", "WSDI", "SW_Supply", "Demand", "Demand_Year", "STATE"]],
        left_on  = huc4_col,
        right_on = "HUC4",
        how="left"
    )

    out_shp = os.path.join(OUTPUT_DIR, f"HUC4_WSDI_IR_SW_{yr}.shp")
    merged.to_file(out_shp)
    print(f"  ✓ HUC4_WSDI_IR_SW_{yr}.shp saved")

print("\n" + "=" * 50)
print("All done! Results are in output/IR_SW/ folder")
print("=" * 50)