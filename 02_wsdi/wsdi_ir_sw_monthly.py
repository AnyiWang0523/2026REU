...
OUTPUT_DIR = os.path.join(BASE_DIR, "output", "IR_SW", "monthly")
...
for yr in range(2000, 2021):
    ...
    if sub.empty:
        continue
    out_csv = os.path.join(OUTPUT_DIR, f"IR_SW_monthly_{yr}.csv")
    sub.to_csv(out_csv, index=False)
    print(f"  ✓ IR_SW_monthly_{yr}.csv saved ({len(sub)} records)")

# Also save complete dataset as single CSV
out_full = os.path.join(OUTPUT_DIR, "IR_SW_monthly_2000_2020.csv")
sw_monthly.to_csv(out_full, index=False)
print(f"\n✓ Full dataset saved: IR_SW_monthly_2000_2020.csv")

print("\n" + "=" * 50)
print("All done! Results are in output/IR_SW/monthly/ folder")
print("=" * 50)